 ...
 * 
 * To run this program, the SdFat library must be installed.
 */
...
// not use the default value, SS. 

const uint8_t accelChipSelect = 6;
...
void loop() {
  ...
  cout << c << endl;
  while (c != 'Y') {
    cout << F("You did not enter 'Y'. Enter 'Y' to continue: \n");
    return;
  }
  ...
  cout << mode << endl;
  while (!strchr("EFQ", mode)) {
    cout << F("Invalid option entered. Options are:\n"
         "E - erase the card and skip formatting.\n"
         "F - erase and then format the card. (recommended)\n"
         "Q - quick format the card without erase.\n"
         "\n"
 ...
 cout << F(
  ...
  for (int i = 0; i< 11; i++){
    ...
    if (isalpha(c) || isdigit(c)) noName[i]=c;
    else noName[i] = ' ';
  }
  Serial.flush();
